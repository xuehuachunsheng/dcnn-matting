...
def knn_matte(img, trimap, mylambda=100):
    ...
    try:
        ...
        import numpy.matlib
        D_script = numpy.matlib.diag(np.ravel(A.sum(axis=1)))
        L = D_script - A
        D = numpy.matlib.diag(np.ravel(all_constraints[:, :, 0]))
        v = np.ravel(foreground[:, :, 0])
        c = 2 * mylambda * np.transpose(v)
        H = 2 * (L + mylambda * D)
...
def main():
    img = scipy.misc.imread('./test_inputs/donkey.png')[:,:,:3]
    trimap = scipy.misc.imread('./test_inputs/donkeyTrimap.png')[:,:,:3]
    alpha = knn_matte(img, trimap)
    scipy.misc.imsave('./test_outputs/donkeyAlpha_knn_matting.png', alpha)
