...
//载入图像并初始化相关变量
void SharedMatting::loadImage(uchar* im, const int height, const int width)
{
    // 将double类型的图片转化为uint8类型的图片

    this->height = height;
    this->width = width;
    this->channels = 3;
    this->step = width*this->channels;
    this->data = im;
...
void SharedMatting::loadTrimap(uchar* trimap, const int height, const int width)
{
    trimap_s = width * 3;
    trimap_c = 3;
    trimap_data = trimap;
...
void SharedMatting::setMatte(uchar* matting)
{
    int h     = height;
    int w     = width;
    int s     = width;
...
//主干方法
void SharedMatting::solveAlpha(uchar* matting)
{
